/**
 * Solicitudes handlers para Cloudflare Workers
 * Adaptado de solicitudes.controller.js
 */
import { addCorsHeaders } from '../middleware/cors.js';

/**
 * GET /api/solicitudes - Obtener todos
 */
export async function getAll(request, env) {
  try {
    const stmt = request.db.prepare('SELECT * FROM solicitudes ORDER BY created_at DESC');
    const result = await stmt.all();
    
    return addCorsHeaders(new Response(JSON.stringify({
      ok: true,
      solicitudes: result.results || []
    }), {
      status: 200,
...
export async function getById(request, env) {
  try {
    ...
    const stmt = request.db.prepare('SELECT * FROM solicitudes WHERE id = ?');
...
/**
 * POST /api/solicitudes - Crear nuevo
 */
...
export async function update(request, env) {
  try {
    ...
    const check = await request.db.prepare('SELECT id FROM solicitudes WHERE id = ?').bind(id).first();
    if (!check) {
      return addCorsHeaders(new Response(JSON.stringify({
        ok: false,
        msg: 'No encontrado'
...
export async function remove(request, env) {
  try {
    ...
    await request.db.prepare('DELETE FROM solicitudes WHERE id = ?').bind(id).run();
