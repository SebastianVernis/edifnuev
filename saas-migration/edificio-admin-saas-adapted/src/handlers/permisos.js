/**
 * Permisos handlers para Cloudflare Workers
 * Adaptado de permisos.controller.js
 */
...
export async function getAll(request, env) {
  try {
    const stmt = request.db.prepare('SELECT * FROM permisos ORDER BY created_at DESC');
...
export async function getById(request, env) {
  try {
    ...
    const stmt = request.db.prepare('SELECT * FROM permisos WHERE id = ?');
...
export async function update(request, env) {
  try {
    ...
    const check = await request.db.prepare('SELECT id FROM permisos WHERE id = ?').bind(id).first();
    if (!check) {
      return addCorsHeaders(new Response(JSON.stringify({
        ok: false,
        msg: 'No encontrado'
...
export async function remove(request, env) {
  try {
    ...
    await request.db.prepare('DELETE FROM permisos WHERE id = ?').bind(id).run();
