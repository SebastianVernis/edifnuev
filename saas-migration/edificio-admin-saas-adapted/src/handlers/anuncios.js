/**
 * Anuncios handlers para Cloudflare Workers
 * Adaptado de anuncios.controller.js
 */
import { addCorsHeaders } from '../middleware/cors.js';

/**
 * GET /api/anuncios - Obtener todos
 */
export async function getAll(request, env) {
  try {
    const stmt = request.db.prepare('SELECT * FROM anuncios ORDER BY created_at DESC');
    const result = await stmt.all();
    
    return addCorsHeaders(new Response(JSON.stringify({
      ok: true,
      anuncios: result.results || []
    }), {
      status: 200,
...
export async function getById(request, env) {
  try {
    ...
    const stmt = request.db.prepare('SELECT * FROM anuncios WHERE id = ?');
...
export async function create(request, env) {
  try {
    ...
    console.error('Error creando anuncios:', error);
    return addCorsHeaders(new Response(JSON.stringify({
      ok: false,
      msg: 'Error en el servidor',
      error: env.ENVIRONMENT === 'development' ? error.message : undefined
...
export async function update(request, env) {
  try {
    ...
    const check = await request.db.prepare('SELECT id FROM anuncios WHERE id = ?').bind(id).first();
    if (!check) {
      return addCorsHeaders(new Response(JSON.stringify({
        ok: false,
        msg: 'No encontrado'
...
export async function remove(request, env) {
  try {
    ...
    await request.db.prepare('DELETE FROM anuncios WHERE id = ?').bind(id).run();
