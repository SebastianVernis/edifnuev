...
const AdminDashboard = (() => {
  ...
  const setupModalCloseButtons = () => {
    // Close buttons
    document
      .querySelectorAll(".modal .close, .modal .modal-cancel")
      .forEach((button) => {
...
const getEditorRoleName = (rolEditor) => {
  switch (rolEditor) {
    case "cuotas":
      return "Editor Cuotas";
    case "presupuestos":
      return "Editor Presupuestos";
    case "gastos":
      return "Editor Gastos";
    case "anuncios":
      return "Editor Anuncios";
    case "solicitudes":
      return "Editor Solicitudes";
    default:
      return rolEditor;
  }
};
    ...
    <div class="modal-overlay" id="editorRoleModal" style="display: block;">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <button class="btn btn-primary" onclick="closeEditorRoleModal()">Cancelar</button>
...
window.saveEditorRole = async (userId) => {
  try {
    ...
    const permisoEscritura =
      document.getElementById("permisoEscritura").checked;
    const permisoAprobacion =
      document.getElementById("permisoAprobacion").checked;
      ...
      <td class="actions">
        ...
        ${
          usuario.rol === "inquilino"
            ? `<button class="btn btn-sm btn-info" onclick="assignEditorRole(${usuario.id})" title="Asignar rol de editor">
        ...
        ${
          usuario.rol !== "superadmin"
            ? `<button class="btn btn-sm btn-danger" onclick="deleteUser(${usuario.id})" title="Eliminar usuario">
...
window.deleteUser = async (userId) => {
  ...
  if (
    confirm(`¿Estás seguro de que quieres eliminar al usuario ${user.nombre}?`)
  ) {
    try {
