...
async function cargarProyectos() {
  try {
    const token =
      localStorage.getItem("edificio_token") || localStorage.getItem("token");
...
function renderProyectos(proyectosData, resumen) {
  ...
  if (proyectosData.length === 0) {
    container.innerHTML =
      '<p class="empty-message">No hay proyectos configurados</p>';
    return;
  }
      ...
      ${proyectosData
        ...
        <div class="proyecto-item" data-id="${p.id}">
          ...
          <div class="proyecto-actions">
            <button class="btn btn-sm btn-primary edit-proyecto" data-id="${p.id}">
...
async function cargarInfoGeneral() {
  try {
    ...
    if (data.ok) {
      const totalDeptos = data.usuarios.filter(
        (u) => u.departamento !== "Admin",
      ).length;
...
function setupEventListeners() {
  ...
  const descargarPoliticasBtn = document.getElementById(
    "descargar-politicas-btn",
  );
...
function abrirModalProyecto(proyectoData = null) {
  ...
  if (proyectoData) {
    ...
    document.getElementById("proyecto-prioridad").value =
      proyectoData.prioridad;
  } else {
    // Nuevo
...
async function cargarPerfilUsuario() {
  try {
    ...
    document.getElementById("perfil-departamento").value =
...
async function guardarPerfil() {
  ...
  if (!currentUser || !currentUser.id) {
    alert(
      "❌ No se pudo identificar el usuario actual. Por favor, inicia sesión nuevamente.",
    );
    return;
  }
  ...
  const passwordConfirmar = document.getElementById(
    "perfil-password-confirmar",
  ).value;
  ...
  try {
    ...
    if (passwordNueva) {
      ...
      if (!passwordData.success) {
        alert(
          "⚠️ Datos actualizados pero error al cambiar contraseña: " +
...
async function cargarInfoEdificio() {
  try {
    ...
    if (data.ok) {
      ...
      document.getElementById("edificio-direccion").value =
        info.direccion || "";
      document.getElementById("edificio-total-unidades").value =
        info.totalUnidades || 20;
      document.getElementById("edificio-cuota-mensual").value =
        info.cuotaMensual || 1500;
      ...
      const porcentajeMoraEl = document.getElementById(
        "edificio-porcentaje-mora",
      );
      ...
      if (reglamentoEl)
        reglamentoEl.value = info.reglamento || info.politicas || "";

      const privacyPolicyEl = document.getElementById(
        "edificio-privacy-policy",
      );
      if (privacyPolicyEl) privacyPolicyEl.value = info.privacyPolicy || "";

      const paymentPoliciesEl = document.getElementById(
        "edificio-payment-policies",
      );
      if (paymentPoliciesEl)
        paymentPoliciesEl.value = info.paymentPolicies || "";

      // Retrocompatibilidad
      const politicasEl = document.getElementById("edificio-politicas");
      if (politicasEl)
        politicasEl.value = info.politicas || info.reglamento || "";
      ...
      const fondoIngresosSelect = document.getElementById(
        "edificio-fondo-ingresos",
      );
...
function renderFondos(funds) {
  ...
  if (funds.length === 0) {
    container.innerHTML =
      '<p class="empty-message">No hay fondos configurados</p>';
    return;
  }
...
async function guardarInfoEdificio() {
  ...
  const diaCorte = parseInt(
    document.getElementById("edificio-dia-corte").value,
  );
  ...
  const porcentajeMora = porcentajeMoraEl
    ? parseFloat(porcentajeMoraEl.value)
    : 2;
  ...
  const fondoIngresosId =
    fondoIngresosEl && fondoIngresosEl.value
      ? parseInt(fondoIngresosEl.value)
      : null;
...
function renderDocumentos(documentos) {
  ...
  if (documentos.length === 0) {
    container.innerHTML =
      '<p class="empty-message">No hay documentos cargados</p>';
    return;
  }
    ...
    <div class="documento-item">
      <div class="documento-item-header">
        ...
        <div class="documento-info">
          ...
          <p style="font-size: 0.8rem; color: #999;">${formatFecha(doc.created_at || doc.fecha)}</p>
        </div>
      </div>
      <div class="documento-actions">
        <button class="btn btn-sm btn-primary" onclick="descargarDocumento('${doc.archivo_url}', '${doc.nombre}')">
