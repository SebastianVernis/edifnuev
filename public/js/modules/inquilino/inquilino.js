...
async function cargarDashboard() {
  try {
    ...
    const cuotasMensuales = cuotasData.filter(
      (c) => c.tipo_cuota === "mensual",
    );
    const cuotaFondoMayor = cuotasData.find(
      (c) => c.tipo_cuota === "fondo_mayor",
    );

    // Cuotas mensuales pendientes
    const cuotasMensualesPendientes = cuotasMensuales.filter((c) => !c.pagado);
    const montoCuotasPendientes = cuotasMensualesPendientes.reduce(
      (sum, c) => sum + c.monto,
      0,
    );
    ...
    if (montoCuotasPendientes > 0) {
      partes.push(
        `${formatCurrency(montoCuotasPendientes)} (${cuotasMensualesPendientes.length} cuota${cuotasMensualesPendientes.length > 1 ? "s" : ""} mensual${cuotasMensualesPendientes.length > 1 ? "es" : ""})`,
      );
    }
    ...
    document.getElementById("misCuotasPendientes").textContent =
      cuotasMensualesPendientes.length;
    document.getElementById("miMontoPendiente").textContent =
      formatCurrency(totalAPagar);
    document.getElementById("desglosePago").textContent = desglose;
    document.getElementById("misSolicitudes").textContent =
...
function mostrarAnunciosImportantes(anunciosData) {
  const anunciosImportantes = anunciosData
    .filter(
      (a) => a.activo && (a.tipo === "urgente" || a.tipo === "mantenimiento"),
    )
    .slice(0, 3);
  ...
  container.innerHTML = anunciosImportantes
    .map(
      (anuncio) => `
...
function mostrarMisCuotas(cuotasFiltradas = null) {
  ...
  const cuotasMensuales = (cuotasFiltradas || misCuotas).filter(
    (c) => c.tipo_cuota === "mensual",
  );

  // Ordenar por mes (fecha de vencimiento)
  const cuotasOrdenadas = cuotasMensuales.sort(
  ...
  tbody.innerHTML = cuotasOrdenadas
    .map((cuota) => {
      const estado = cuota.pagado
        ? "pagado"
        : new Date(cuota.fecha_vencimiento) < new Date()
          ? "vencido"
          : "pendiente";
      const mesNombre = cuota.concepto
        .replace("Cuota Mensual ", "")
        .split(" ")[0];
...
function mostrarCuotasTipo(tipo) {
  ...
  document
    .getElementById(tipo === "mensual" ? "tabMensual" : "tabFondo")
    .classList.add("active");

  // Cambiar contenido activo
  document
    .querySelectorAll(".cuotas-content")
    .forEach((content) => content.classList.remove("active"));
  document
    .getElementById(tipo === "mensual" ? "cuotasMensuales" : "fondoMayor")
    .classList.add("active");
...
function actualizarResumenCuotas() {
  ...
  const totalPendienteMensual = cuotasMensualesPendientes.reduce(
    (sum, c) => sum + c.monto,
    0,
  );

  document.getElementById("cuotasMensualesPendientes").textContent =
    formatCurrency(totalPendienteMensual);
  document.getElementById("cuotasMensualesPagadas").textContent =
    `${cuotasMensualesPagadas.length}/12`;

  // Estado del fondo de gastos mayores
  const fondoMayor = cuotasFondo[0];
  if (fondoMayor) {
    const montoPagado = fondoMayor.monto_pagado || 0;
    document.getElementById("fondoGastosMayores").textContent =
      `$${montoPagado} / $5,000`;
  }
  ...
  if (cuotasMensualesPendientes.length > 0) {
    ...
    if (cuotasFuturas.length > 0) {
      ...
      const nombreMes = cuotasFuturas[0].concepto
        .replace("Cuota Mensual ", "")
        .split(" ")[0];
      document.getElementById("proximoVencimiento").textContent =
        `${nombreMes} - ${formatDate(proximaFecha)}`;
    } else {
      const cuotaVencida = cuotasMensualesPendientes.sort(
        (a, b) => new Date(a.fecha_vencimiento) - new Date(b.fecha_vencimiento),
      )[0];
      const nombreMes = cuotaVencida.concepto
        .replace("Cuota Mensual ", "")
        .split(" ")[0];
      document.getElementById("proximoVencimiento").textContent =
        `${nombreMes} - VENCIDA`;
...
function mostrarAnuncios(filtro = "todos") {
  ...
  container.innerHTML = anunciosFiltrados
    .map(
      (anuncio) => `
        <div class="announcement-card ${anuncio.tipo}">
            <h4>${anuncio.titulo}</h4>
...
function mostrarMisSolicitudes() {
  ...
  tbody.innerHTML = misSolicitudes
    .map(
      (solicitud) => `
        <tr>
            ...
            <td>
                <button onclick="verSolicitud(${solicitud.id})" class="btn-primary">Ver</button>
    ...
    .map(
      (presupuesto) => `
        <tr>
            ...
            <td>
                <button onclick="verPresupuestoDetalle(${presupuesto.id})" class="btn-primary">Ver Detalle</button>
            </td>
        </tr>
    `,
    )
...
function filtrarMisCuotas() {
  ...
  const cuotasFiltradas = misCuotas.filter((cuota) => {
    ...
    const estadoCuota = cuota.pagado
      ? "pagado"
      : new Date(cuota.fecha_vencimiento) < new Date()
        ? "vencido"
        : "pendiente";
...
function mostrarEstadoFondoMayor(data) {
  ...
  if (data.parcialidades.length > 0) {
    tbody.innerHTML = data.parcialidades
      .map(
        (p) => `
            <tr>
...
function mostrarFormPropuesta() {
  ...
  modalBody.innerHTML = `
        ...
        <form id="formPropuesta">
            ...
            <div class="form-buttons">
                ...
                <button type="button" onclick="cerrarModal()" class="btn-primary">Cancelar</button>
            </div>
        </form>
  ...
  document
    .getElementById("formPropuesta")
    .addEventListener("submit", async (e) => {
      e.preventDefault();

      // Por ahora solo mostrar mensaje
      alert(
        "Funcionalidad de propuestas estará disponible próximamente. Su propuesta ha sido registrada para desarrollo futuro.",
      );
      cerrarModal();
